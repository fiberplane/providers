...
use anyhow::bail;
use clap::Parser;
use fiberplane_ci::{commands::versions::*, utils::*, TaskResult};
...
fn handle_set_version(args: &SetVersionArgs) -> TaskResult {
    ...
    if let Some(crate_name) = args.crate_name.as_ref() {
        if let Some(crate_dir) = get_crate_dir_from_name(&all_crate_dirs, crate_name) {
            set_version(format!("{crate_dir}/Cargo.toml"), args)?;
        }

        println!("{SUCCESS}Crate version updated.");
    } else {
